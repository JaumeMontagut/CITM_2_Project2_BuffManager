...
Image* Module_UI::CreateImage(iPoint position, SDL_Rect rect, Gui_Listener* listener)
{
	Image* object = new Image(position, rect, atlas, listener);
	object->SetAnchor(screen);
	objects_list.push_back(object);
	return object;
}

Button* Module_UI::CreateButton(iPoint position, SDL_Rect rect, Gui_Listener* listener)
{
	Button* object = new Button(position, rect, atlas, listener);
	object->SetAnchor(screen);
	objects_list.push_back(object);
	return object;
}
